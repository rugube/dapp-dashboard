import {
  Button,
  FormControl,
  InputLabel,
  MenuItem,
  FormHelperText,
  Select,
} from "@mui/material";
import "./FrameComponent.css";
const FrameComponent = () => {
  return (
    <div className="parent">
      <form className="form">
        <div className="rectangle-parent">
          <div className="instance-child" />
          <div className="frame-parent">
            <a className="image-3099-parent" href="#">
              <img
                className="image-3099-icon"
                alt=""
                src="/image-3099@2x.png"
              />
              <div className="brc-20-dao">BRC-20 DAO</div>
            </a>
            <div className="frame-group">
              <a className="frame-container" href="#" target="_blank">
                <div className="wrapper">
                  <div className="div">{`{  }`}</div>
                </div>
                <b className="brc-20">BRC-20</b>
              </a>
              <a className="frame-container" href="#" target="_blank">
                <img className="wrapper" alt="" src="/frame.svg" />
                <b className="launchpad">Launchpad</b>
              </a>
            </div>
            <a className="label" href="#" target="_blank">
              <div className="coming-soon">coming soon</div>
            </a>
            <img className="group-child" alt="" src="/ellipse-2575.svg" />
          </div>
          <img className="instance-item" alt="" src="/ellipse-2575.svg" />
          <div className="social-link-parent">
            <a
              className="social-link"
              href="https://twitter.com"
              target="_blank"
            >
              <img className="social-link-child" alt="" src="/group-1510.svg" />
            </a>
            <a
              className="social-link"
              href="htps://discord.com"
              target="_blank"
            >
              <img className="social-link-child" alt="" src="/frame-5608.svg" />
            </a>
            <Button variant="contained" color="primary">
              Connect Wallet
            </Button>
          </div>
        </div>
        <div className="frame-div">
          <div className="thstickytop-parent">
            <div className="thstickytop">
              <div className="brc-20-dao">#</div>
            </div>
            <a className="thstickytop1" href="#" target="_blank">
              <div className="brc-20-dao">Name</div>
            </a>
            <a className="thstickytop2" href="#" target="_blank">
              <div className="brc-20-dao">Price</div>
            </a>
            <a className="thstickytop3" href="#" target="_blank">
              <div className="brc-20-dao">24h %</div>
            </a>
            <a className="thstickytop4" href="#" target="_blank">
              <div className="market-cap-parent">
                <div className="market-cap">Market Cap</div>
                <img className="wrapper" alt="" src="/frame1.svg" />
              </div>
            </a>
            <a className="thstickytop4" href="#" target="_blank">
              <div className="brc-20-dao">Volume(24h)</div>
            </a>
            <a className="thstickytop6" href="#" target="_blank">
              <div className="brc-20-dao">Supply</div>
            </a>
            <a className="thstickytop2" href="#" target="_blank">
              <div className="brc-20-dao">Holders</div>
            </a>
          </div>
          <a className="tdh2d-f1880f84-parent" href="#" target="_blank">
            <div className="tdh2d-f1880f84">
              <div className="brc-20-dao">1</div>
            </div>
            <div className="tdh2d-c378f017">
              <div className="divsc-aef7b723-0">
                <div className="image-3120-parent">
                  <img
                    className="image-3120-icon"
                    alt=""
                    src="/image-3120@2x.png"
                  />
                  <b className="b">$</b>
                </div>
                <div className="brc-20-dao">ordi</div>
              </div>
            </div>
            <div className="tdh2d-dd5d7225">
              <div className="brc-20-dao">$14.92</div>
            </div>
            <div className="tdh2d-2e7f64ed">
              <div className="div4">5.819%</div>
            </div>
            <div className="tdh2d-6c00a7ab">
              <div className="brc-20-dao">$520,648,120.67</div>
            </div>
            <div className="tdh2d-e5ceb692">
              <div className="divsc-aef7b723-01">
                <div className="brc-20-dao">$11,405,290.58</div>
              </div>
            </div>
            <div className="tdh2d-74ed3dab">
              <div className="divsc-aef7b723-02">
                <div className="div7">
                  <span className="span">19,372,400 (</span>
                  <span className="span1">100%</span>
                  <span className="span">)</span>
                </div>
                <div className="divsc-4ff2400b-0">
                  <div className="divsc-4ff2400b-1" />
                </div>
              </div>
            </div>
            <div className="tdh2d-77430025">
              <div className="div8">8,000</div>
            </div>
          </a>
          <a className="tdh2d-f1880f84-group" href="#" target="_blank">
            <div className="tdh2d-f1880f84">
              <div className="brc-20-dao">2</div>
            </div>
            <div className="tdh2d-c378f017">
              <div className="divsc-aef7b723-0">
                <div className="image-3120-parent">
                  <img
                    className="image-3120-icon"
                    alt=""
                    src="/image-31201@2x.png"
                  />
                  <b className="b">$</b>
                </div>
                <div className="brc-20-dao">VMPX</div>
              </div>
            </div>
            <div className="tdh2d-dd5d7225">
              <div className="brc-20-dao">$14.92</div>
            </div>
            <div className="tdh2d-2e7f64ed">
              <div className="div4">5.819%</div>
            </div>
            <div className="tdh2d-6c00a7ab">
              <div className="brc-20-dao">$520,648,120.67</div>
            </div>
            <div className="tdh2d-e5ceb692">
              <div className="divsc-aef7b723-01">
                <div className="brc-20-dao">$11,405,290.58</div>
              </div>
            </div>
            <div className="tdh2d-74ed3dab">
              <div className="divsc-aef7b723-02">
                <div className="div7">
                  <span className="span">19,372,400 (</span>
                  <span className="span1">100%</span>
                  <span className="span">)</span>
                </div>
                <div className="divsc-4ff2400b-0">
                  <div className="divsc-4ff2400b-1" />
                </div>
              </div>
            </div>
            <div className="tdh2d-77430025">
              <div className="div8">8,000</div>
            </div>
          </a>
          <a className="tdh2d-f1880f84-container" href="#" target="_blank">
            <div className="tdh2d-f1880f84">
              <div className="brc-20-dao">3</div>
            </div>
            <div className="tdh2d-c378f017">
              <div className="divsc-aef7b723-0">
                <div className="image-3120-parent">
                  <img
                    className="image-3120-icon"
                    alt=""
                    src="/image-31202@2x.png"
                  />
                  <b className="b">$</b>
                </div>
                <div className="brc-20-dao">pepe</div>
              </div>
            </div>
            <div className="tdh2d-dd5d7225">
              <div className="brc-20-dao">$14.92</div>
            </div>
            <div className="tdh2d-2e7f64ed">
              <div className="div18">-5.819%</div>
            </div>
            <div className="tdh2d-6c00a7ab">
              <div className="brc-20-dao">$520,648,120.67</div>
            </div>
            <div className="tdh2d-e5ceb692">
              <div className="divsc-aef7b723-01">
                <div className="brc-20-dao">$11,405,290.58</div>
              </div>
            </div>
            <div className="tdh2d-74ed3dab">
              <div className="divsc-aef7b723-02">
                <div className="div7">
                  <span className="span">19,372,400 (</span>
                  <span className="span1">100%</span>
                  <span className="span">)</span>
                </div>
                <div className="divsc-4ff2400b-0">
                  <div className="divsc-4ff2400b-1" />
                </div>
              </div>
            </div>
            <div className="tdh2d-77430025">
              <div className="div8">8,000</div>
            </div>
          </a>
          <a className="tdh2d-f1880f84-parent1" href="#" target="_blank">
            <div className="tdh2d-f1880f84">
              <div className="brc-20-dao">4</div>
            </div>
            <div className="tdh2d-c378f017">
              <div className="divsc-aef7b723-0">
                <div className="image-3120-parent">
                  <div className="image-3120" />
                  <b className="b3">$</b>
                </div>
                <div className="brc-20-dao">$HMC</div>
              </div>
            </div>
            <div className="tdh2d-dd5d7225">
              <div className="brc-20-dao">$14.92</div>
            </div>
            <div className="tdh2d-2e7f64ed">
              <div className="div4">5.819%</div>
            </div>
            <div className="tdh2d-6c00a7ab">
              <div className="brc-20-dao">$520,648,120.67</div>
            </div>
            <div className="tdh2d-e5ceb692">
              <div className="divsc-aef7b723-01">
                <div className="brc-20-dao">$11,405,290.58</div>
              </div>
            </div>
            <div className="tdh2d-74ed3dab">
              <div className="divsc-aef7b723-02">
                <div className="div7">
                  <span className="span">19,372,400 (</span>
                  <span className="span1">100%</span>
                  <span className="span">)</span>
                </div>
                <div className="divsc-4ff2400b-0">
                  <div className="divsc-4ff2400b-1" />
                </div>
              </div>
            </div>
            <div className="tdh2d-77430025">
              <div className="div8">8,000</div>
            </div>
          </a>
          <a className="tdh2d-f1880f84-container" href="#">
            <div className="tdh2d-f1880f84">
              <div className="brc-20-dao">5</div>
            </div>
            <div className="tdh2d-c378f017">
              <div className="divsc-aef7b723-0">
                <div className="image-3120-parent">
                  <img
                    className="image-3120-icon"
                    alt=""
                    src="/image-31203@2x.png"
                  />
                  <b className="b">$</b>
                </div>
                <div className="brc-20-dao">$OG$</div>
              </div>
            </div>
            <div className="tdh2d-dd5d7225">
              <div className="brc-20-dao">$14.92</div>
            </div>
            <div className="tdh2d-2e7f64ed">
              <div className="div4">5.819%</div>
            </div>
            <div className="tdh2d-6c00a7ab">
              <div className="brc-20-dao">$520,648,120.67</div>
            </div>
            <div className="tdh2d-e5ceb692">
              <div className="divsc-aef7b723-01">
                <div className="brc-20-dao">$11,405,290.58</div>
              </div>
            </div>
            <div className="tdh2d-74ed3dab">
              <div className="divsc-aef7b723-02">
                <div className="div7">
                  <span className="span">19,372,400 (</span>
                  <span className="span1">100%</span>
                  <span className="span">)</span>
                </div>
                <div className="divsc-4ff2400b-0">
                  <div className="divsc-4ff2400b-1" />
                </div>
              </div>
            </div>
            <div className="tdh2d-77430025">
              <div className="div8">8,000</div>
            </div>
          </a>
          <a className="tdh2d-f1880f84-parent3" href="#" target="_blank">
            <div className="tdh2d-f1880f84">
              <div className="brc-20-dao">6</div>
            </div>
            <div className="tdh2d-c378f017">
              <div className="divsc-aef7b723-0">
                <div className="image-3120-parent">
                  <div className="image-3120" />
                  <b className="b3">B</b>
                </div>
                <div className="brc-20-dao">blur</div>
              </div>
            </div>
            <div className="tdh2d-dd5d7225">
              <div className="brc-20-dao">$14.92</div>
            </div>
            <div className="tdh2d-2e7f64ed">
              <div className="div4">5.819%</div>
            </div>
            <div className="tdh2d-6c00a7ab">
              <div className="brc-20-dao">$520,648,120.67</div>
            </div>
            <div className="tdh2d-e5ceb692">
              <div className="divsc-aef7b723-01">
                <div className="brc-20-dao">$11,405,290.58</div>
              </div>
            </div>
            <div className="tdh2d-74ed3dab">
              <div className="divsc-aef7b723-02">
                <div className="div7">
                  <span className="span">19,372,400 (</span>
                  <span className="span1">100%</span>
                  <span className="span">)</span>
                </div>
                <div className="divsc-4ff2400b-0">
                  <div className="divsc-4ff2400b-1" />
                </div>
              </div>
            </div>
            <div className="tdh2d-77430025">
              <div className="div8">8,000</div>
            </div>
          </a>
          <a className="tdh2d-f1880f84-container" href="#" target="_blank">
            <div className="tdh2d-f1880f84">
              <div className="brc-20-dao">7</div>
            </div>
            <div className="tdh2d-c378f017">
              <div className="divsc-aef7b723-0">
                <div className="image-3120-parent">
                  <img
                    className="image-3120-icon"
                    alt=""
                    src="/image-31204@2x.png"
                  />
                  <b className="b">$</b>
                </div>
                <div className="brc-20-dao">meme</div>
              </div>
            </div>
            <div className="tdh2d-dd5d7225">
              <div className="brc-20-dao">$14.92</div>
            </div>
            <div className="tdh2d-2e7f64ed">
              <div className="div4">5.819%</div>
            </div>
            <div className="tdh2d-6c00a7ab">
              <div className="brc-20-dao">$520,648,120.67</div>
            </div>
            <div className="tdh2d-e5ceb692">
              <div className="divsc-aef7b723-01">
                <div className="brc-20-dao">$11,405,290.58</div>
              </div>
            </div>
            <div className="tdh2d-74ed3dab">
              <div className="divsc-aef7b723-02">
                <div className="div7">
                  <span className="span">19,372,400 (</span>
                  <span className="span1">100%</span>
                  <span className="span">)</span>
                </div>
                <div className="divsc-4ff2400b-0">
                  <div className="divsc-4ff2400b-1" />
                </div>
              </div>
            </div>
            <div className="tdh2d-77430025">
              <div className="div8">8,000</div>
            </div>
          </a>
          <a className="tdh2d-f1880f84-parent3" href="#" target="_blank">
            <div className="tdh2d-f1880f84">
              <div className="brc-20-dao">8</div>
            </div>
            <div className="tdh2d-c378f017">
              <div className="divsc-aef7b723-0">
                <div className="image-3120-parent">
                  <img
                    className="image-3120-icon"
                    alt=""
                    src="/image-31205@2x.png"
                  />
                  <b className="b">$</b>
                </div>
                <div className="brc-20-dao">piza</div>
              </div>
            </div>
            <div className="tdh2d-dd5d7225">
              <div className="brc-20-dao">$14.92</div>
            </div>
            <div className="tdh2d-2e7f64ed">
              <div className="div4">5.819%</div>
            </div>
            <div className="tdh2d-6c00a7ab">
              <div className="brc-20-dao">$520,648,120.67</div>
            </div>
            <div className="tdh2d-e5ceb692">
              <div className="divsc-aef7b723-01">
                <div className="brc-20-dao">$11,405,290.58</div>
              </div>
            </div>
            <div className="tdh2d-74ed3dab">
              <div className="divsc-aef7b723-02">
                <div className="div7">
                  <span className="span">19,372,400 (</span>
                  <span className="span1">100%</span>
                  <span className="span">)</span>
                </div>
                <div className="divsc-4ff2400b-0">
                  <div className="divsc-4ff2400b-1" />
                </div>
              </div>
            </div>
            <div className="tdh2d-77430025">
              <div className="div8">8,000</div>
            </div>
          </a>
          <a className="tdh2d-f1880f84-container" href="#" target="_blank">
            <div className="tdh2d-f1880f84">
              <div className="brc-20-dao">9</div>
            </div>
            <div className="tdh2d-c378f017">
              <div className="divsc-aef7b723-0">
                <div className="image-3120-parent">
                  <img
                    className="image-3120-icon"
                    alt=""
                    src="/image-31206@2x.png"
                  />
                  <b className="b">$</b>
                </div>
                <div className="brc-20-dao">nals</div>
              </div>
            </div>
            <div className="tdh2d-dd5d7225">
              <div className="brc-20-dao">$14.92</div>
            </div>
            <div className="tdh2d-2e7f64ed">
              <div className="div4">5.819%</div>
            </div>
            <div className="tdh2d-6c00a7ab">
              <div className="brc-20-dao">$520,648,120.67</div>
            </div>
            <div className="tdh2d-e5ceb692">
              <div className="divsc-aef7b723-01">
                <div className="brc-20-dao">$11,405,290.58</div>
              </div>
            </div>
            <div className="tdh2d-74ed3dab">
              <div className="divsc-aef7b723-02">
                <div className="div7">
                  <span className="span">19,372,400 (</span>
                  <span className="span1">100%</span>
                  <span className="span">)</span>
                </div>
                <div className="divsc-4ff2400b-0">
                  <div className="divsc-4ff2400b-1" />
                </div>
              </div>
            </div>
            <div className="tdh2d-77430025">
              <div className="div8">8,000</div>
            </div>
          </a>
          <a className="tdh2d-f1880f84-parent3" href="#" target="_blank">
            <div className="tdh2d-f1880f84">
              <div className="brc-20-dao">10</div>
            </div>
            <div className="tdh2d-c378f017">
              <div className="divsc-aef7b723-0">
                <div className="image-3120-parent">
                  <img
                    className="image-3120-icon"
                    alt=""
                    src="/image-31207@2x.png"
                  />
                  <b className="b">$</b>
                </div>
                <div className="brc-20-dao">domo</div>
              </div>
            </div>
            <div className="tdh2d-dd5d7225">
              <div className="brc-20-dao">$14.92</div>
            </div>
            <div className="tdh2d-2e7f64ed">
              <div className="div4">5.819%</div>
            </div>
            <div className="tdh2d-6c00a7ab">
              <div className="brc-20-dao">$520,648,120.67</div>
            </div>
            <div className="tdh2d-e5ceb692">
              <div className="divsc-aef7b723-01">
                <div className="brc-20-dao">$11,405,290.58</div>
              </div>
            </div>
            <div className="tdh2d-74ed3dab">
              <div className="divsc-aef7b723-02">
                <div className="div7">
                  <span className="span">19,372,400 (</span>
                  <span className="span1">100%</span>
                  <span className="span">)</span>
                </div>
                <div className="divsc-4ff2400b-0">
                  <div className="divsc-4ff2400b-1" />
                </div>
              </div>
            </div>
            <div className="tdh2d-77430025">
              <div className="div8">8,000</div>
            </div>
          </a>
        </div>
        <div className="frame-parent1">
          <div className="divsc-e20acb0c-0-parent">
            <input
              className="divsc-e20acb0c-0"
              type="text"
              placeholder="Search name"
            />
            <div className="divsc-c03070ee-2" />
            <div className="buttonsc-44910c32-0">
              <div className="all">All</div>
            </div>
            <a className="in-progress" href="#" target="_blank">
              In-Progress
            </a>
            <a className="in-progress" href="#" target="_blank">
              Completed
            </a>
          </div>
          <button className="divsc-aef7b723-030">
            <div className="sort-by">Sort by</div>
            <div className="divsc-aef7b723-0-wrapper">
              <FormControl
                className="divsc-aef7b723-031"
                sx={{ width: 139 }}
                variant="outlined"
              >
                <InputLabel color="secondary">Market Cap</InputLabel>
                <Select color="secondary" size="medium" label="Market Cap">
                  <MenuItem value="Market Cap">Market Cap</MenuItem>
                  <MenuItem value="Name">Name</MenuItem>
                  <MenuItem value="Holders">Holders</MenuItem>
                </Select>
                <FormHelperText />
              </FormControl>
            </div>
          </button>
        </div>
        <div className="total-market-cap-container">
          <span className="span">{`Total Market Cap `}</span>
          <span className="span1">$539,927,431</span>
        </div>
        <img className="image-3121-icon" alt="" src="/image-3121@2x.png" />
        <div className="brc-20-tokens-parent">
          <b className="brc-20-tokens">BRC-20 Tokens</b>
          <b className="total">
            <span className="span">(</span>
            <span className="span1">14,1552</span>
            <span className="span"> total)</span>
          </b>
        </div>
        <img className="child" alt="" src="/rectangle-22828@2x.png" />
      </form>
    </div>
  );
};

export default FrameComponent;
